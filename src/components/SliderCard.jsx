...
export class SliderCard extends Component {
    ...
    slider() {
        let container=this.state.sliderContainer.current
    ...
    componentDidMount() {
        ...
        setInterval(() => {
            ...
            if (newCount < this.state.sliderContainer.current.children.length - 4) {
                // this.setState({
    ...
    rightIcon() {
        let rightCount=this.state.count;

        if (rightCount < this.state.sliderContainer.current.children.length - 4) {
    ...
    render() {
        const { sliderArr } = this.props
        const { sliderContainer } = this.state

        console.log(sliderContainer);
        return (
            <div class="slider_container">
                <div class="left_container">
                    <svg class="left_icon1" xmlns="http://www.w3.org/2000/svg" width="15" height="29"
                ...
                <div class="center_container" id="popular_movie" ref={sliderContainer}>
                    {
                        sliderArr.map((x, i) => {
                            return <SliderInnerCard key={i} data={x} />
                        })
                    }
                </div>
                <div onClick={this.rightIcon.bind(this)} class="right_container">
                    <svg class="right_icon1" xmlns="http://www.w3.org/2000/svg" width="15" height="29"
                        viewBox="0 0 15 29" fill="none">
                        <path
